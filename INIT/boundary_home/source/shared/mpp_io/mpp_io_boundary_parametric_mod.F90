...
subroutine write_boundary_parametric ( ubot, vbot, utop, vtop, pgfx, pgfy, &
         ...
         x_center, y_center, x_corner, y_corner, utx, uty, n, rmax, wsmax, &
...
!    define the attributes for var
     ...
     retval = nf_def_var(ncid, 'n', NF_REAL, 1, rec_dimid, n_varid )
     if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_parametric/nf_def_var/n_varid' )
     ...
     var_units = ''
     retval = nf_put_att_text(ncid, n_varid, UNITS, len(TRIM(var_units)), var_units)
     if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_parametric/nf_put_att_text/n_units' )
...
!  put var data 
   ...
   retval = nf_put_vara_REAL(ncid, n_varid, recs, 1, n)
   if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_parametric/nf_put_var_REAL/n' )
